...
export default function Home() {
  ...
  useEffect(() => {
    axios
      ...
      .then((res) =>
        setServices(
          (res.data.data || []).filter((s) => s.isAvailable).slice(0, 3),
        ),
      );
    axios
      .get("/api/feedback")
      .then((res) => setFeedbacks(res.data.data?.slice(0, 3) || []));
  }, []);
  ...
  const features = [
    {
      ...
      desc: "Our team holds certifications from major brands with years of hands-on experience.",
    },
    {
      icon: <FaBolt className="text-yellow-500 text-5xl" />,
      title: "Fast Turnaround",
      desc: "Most repairs completed within 1-3 hours.",
    },
    {
      icon: <FaShieldAlt className="text-green-600 text-5xl" />,
      title: "6-Month Warranty",
      desc: "Every repair comes with a 6-month warranty.",
    },
    {
      icon: <FaRupeeSign className="text-emerald-600 text-5xl" />,
      title: "Transparent Pricing",
      desc: "No hidden charges. Upfront quote before we begin.",
    },
  ];

  const highlights = ["OEM Parts", "6-Month Warranty", "No Fix, No Fee"];
    ...
    <div>
      ...
      <section className="relative min-h-screen flex items-center pt-16 overflow-hidden bg-gradient-to-br from-white via-blue-50/30 to-white dark:from-navy dark:via-navy dark:to-navy-light">
        <div
          ...
          style={{
            background:
              "radial-gradient(ellipse at 20% 50%, rgba(59,130,246,0.07) 0%, transparent 60%), radial-gradient(ellipse at 80% 20%, rgba(245,158,11,0.05) 0%, transparent 50%)",
          }}
        />
        <div className="max-w-6xl mx-auto px-6 py-20">
          ...
          <p className="text-lg text-slate-500 dark:text-slate-400 max-w-xl mb-10 leading-relaxed">
            Professional mobile repair services for all brands. Screen, battery,
            water damage, software — fixed with a 6-month warranty.
          </p>
      ...
      <section className="bg-slate-100 dark:bg-navy-light border-y border-slate-200 dark:border-white/[0.06] py-10">
        <div className="max-w-6xl mx-auto px-6 grid grid-cols-2 md:grid-cols-4 gap-6 text-center">
          {stats.map((s, i) => (
            <div key={i}>
              <div className="font-display font-bold text-3xl text-blue-600 dark:text-blue-400 mb-1">
                {s.number}
              </div>
      ...
      <section className="py-20 bg-white dark:bg-navy">
        <div className="max-w-6xl mx-auto px-6">
          ...
          <h2 className="font-display font-bold text-4xl text-slate-900 dark:text-white mb-3">
            Our Repair Services
          </h2>
      ...
      <section className="py-20 bg-slate-50 dark:bg-navy-light">
        <div className="max-w-6xl mx-auto px-6">
          ...
          <h2 className="font-display font-bold text-4xl text-slate-900 dark:text-white mb-12">
            Repair Done Right
          </h2>
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
            {features.map((f, i) => (
              <div
                key={i}
                className="text-center p-8 rounded-2xl bg-white dark:bg-navy border border-slate-200 dark:border-white/[0.06] hover:border-blue-200 dark:hover:border-blue-500/30 hover:-translate-y-1 hover:shadow-xl hover:shadow-blue-500/5 transition-all duration-300"
              >
      ...
      <section className="py-20 relative overflow-hidden bg-gradient-to-r from-cyan-700 via-cyan-600 to-cyan-800">
        <div
          ...
          style={{
            background:
              "radial-gradient(circle at 50%, rgba(255,255,255,0.06), transparent 60%)",
          }}
        />
        <div className="max-w-6xl mx-auto px-6 text-center relative">
          ...
          <p className="text-blue-100 text-lg mb-8">
            Book an appointment online and get a free diagnostic check
          </p>
          <Link to="/appointment" className="btn-amber text-base px-10 py-4">

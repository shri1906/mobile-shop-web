...
export default function Feedback() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      toast.success(err.response?.data?.message || "Submission failed");
    } finally {
      setLoading(false);
    }
  };
          ...
          <div className="lg:col-span-2 space-y-4">
            ...
            {feedbacks.length === 0 ? (
              ...
              feedbacks.map((fb) => (
                <div
                  key={fb._id}
                  className="p-6 rounded-2xl bg-slate-50 dark:bg-navy-light border border-slate-200 dark:border-white/[0.06] hover:border-blue-200 dark:hover:border-blue-500/30 transition-all duration-300"
                >
                ...
                <div className="flex items-center gap-1">
                  {[1, 2, 3, 4, 5].map((n) => (
                    <button
                      ...
                      className={`text-3xl transition-all duration-150 hover:scale-110 ${n <= (hover || form.rating) ? "text-amber-400" : "text-slate-300 dark:text-slate-600"}`}
                    >
                      ★
                    </button>
                  ))}

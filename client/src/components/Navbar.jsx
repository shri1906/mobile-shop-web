    ...
    <nav className={`fixed top-0 left-0 right-0 z-50 transition-all duration-300
      ...
      <div className="max-w-6xl mx-auto px-6 flex items-center justify-between py-4">
        ...
        <div className="flex md:hidden items-center gap-2">
          ...
          <button onClick={() => setMenuOpen(!menuOpen)}
            ...
            <span className={`block w-5 h-0.5 bg-slate-700 dark:bg-slate-300 transition-all duration-300 ${menuOpen ? 'rotate-45 translate-y-2' : ''}`} />
      ...
      {menuOpen && (
        <div className="md:hidden bg-white dark:bg-navy-light border-t border-slate-200 dark:border-white/[0.06] px-6 py-4 flex flex-col gap-1">
          ...
          <Link to="/appointment" className="btn-primary mt-2 justify-center">Book Now</Link>
        </div>
      )}
    </nav>
  );
